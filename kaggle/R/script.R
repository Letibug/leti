...
cat(paste('We will infer their values for **embarkment** based on present data that we can imagine may be relevant: **passenger class** and **fare**. We see that they paid<b> $', 
          full[c(62, 830), 'Fare'][[1]][1], '</b>and<b> $', full[c(62, 830), 'Fare'][[1]][2], '</b>respectively and their classes are<b>', full[c(62, 830), 'Pclass'][[1]][1], '</b>and<b>', full[c(62, 830), 'Pclass'][[1]][2], '</b>. So from where did they embark?'))
...
varImportance <- data.frame(Variables = row.names(importance), 
                            Importance = as.numeric(round(importance[ ,1],2)))
index<-order(varImportance$Importance, decreasing=TRUE)
varImportance$Variables[index]
rank<-order(index)
