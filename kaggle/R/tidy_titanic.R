...
#              A p-value of just above 5\% does normally not count as significant. And even if it were just below 5\% there are several other variables here that could influence the statistics. Therefore, it's better to look at the larger numbers for a useful signal.
              ...
              #   ```{r}
                ...
                combine <- combine %>%
                  mutate(title = fct_lump(title_orig, n=4),
                         fare_eff = Fare/ticket_group)
                ...
                # For the *age\_known* feature we find the interesting effect that for women it appears to decrease survival rates throughout the *Pclasses*. Moreover, while for men in *Pclass* 1 or 3 a known age seems to help survival it is the opposite for *Pclass* 2. Since *Pclass == 3* males make up the largest part of the passengers it is not surprising that their survival advantage for *age\_known == 1* translates to a global one. There might be a more complex relation here between different parameters, but we will check this at the modelling stage.
                # 
                # 
                ...
                # ## Cabin- and ticket-related parameters: *deck*, *cabin\_known*, *ttype*, *bad\_ticket*  {.tabset}
                # 
